    ...
    try {
        ...
        while (true) {
            ...
            for (let line of lines) {
                ...
                // Explicitly cast to Number to avoid NaN issues
        ...
        for (let i = 0; i < rowCount; i++) {
            ...
            if (presData[i]) {
                presData[i].split('|').forEach(o => { 
                    const cl = o.trim(); 
                    if(cl) { 
                        relations[bu].offers.add(cl); 
                        relations[bu].types.add(cl.split('-')[0]); 
                    }
                });
            }
        }
...
function calculate(exclOff, exclTyp, f, metricKey, isSimulation) {
    ...
    for (let i = 0; i < rowCount; i++) {
        ...
        // UNIQUE SEGMENT DEDUPLICATION
        // Ensure segKey is a robust primitive string
    ...
    return res.map(r => ({ 
        eligRate: r.h > 0 ? (r.elig/r.h)*100 : 0, 
        offRate: r.elig > 0 ? (r.ext/r.elig)*100 : 0, 
        accRate: r.elig > 0 ? (r.acc/r.elig)*100 : 0, 
        convRate: r.ext > 0 ? (r.acc/r.ext)*100 : 0, 
        h: r.h, elig: r.elig, ext: r.ext, acc: r.acc, 
        typeStatsNum: r.typeStatsNum, comboStatsNum: r.comboStatsNum 
    }));
}
