/** worker.js - v2.2 Strictly Isolated Logic **/
...
async function streamCSV(url) {
    ...
    while (true) {
        ...
        for (let line of lines) {
            ...
            const getIdx = (v, m) => { let clean = v.trim(); let i = m.indexOf(clean); if (i === -1) { m.push(clean); return m.length - 1; } return i; };
            mthCol[rowCount] = getIdx(c[0], dateMap); buCol[rowCount] = getIdx(c[1], buMap); tierCol[rowCount] = getIdx(c[2], tierMap); siteCol[rowCount] = getIdx(c[3], siteMap);
    ...
    for (let i = 0; i < rowCount; i++) {
        ...
        relations[bu].tiers.add(tierMap[tierCol[i]]); relations[bu].sites.add(siteMap[siteCol[i]]);
        if (presData[i]) presData[i].split('|').forEach(o => { const cl = o.trim(); if(cl) { relations[bu].offers.add(cl); relations[bu].types.add(cl.split('-')[0]); } });
    }
...
function calculate(exclOff, exclTyp, f, metricKey, isSimulation) {
    ...
    const seen = new Set(); // Reset for every run to ensure fresh denominator

    for (let i = 0; i < rowCount; i++) {
        if (buCol[i] !== buIdx || !tIdx.has(tierCol[i]) || !sIdx.has(siteCol[i])) continue;
        const d = mthCol[i], r = res[d];
        
        // --- Denominator Deduplication ---
        ...
        // --- Exclusions apply ONLY to Simulation ---
        ...
        if (act.length > 0) {
            ...
            // Stats for PoP drivers (Logic is always based on selected metric stage)
            const isSuccess = (metricKey === 'eligRate') ? true : (metricKey === 'convRate') ? accAct : extAct;
            if (isSuccess) {
                const ck = act.sort().join(' | '); r.comboStatsNum[ck] = (r.comboStatsNum[ck] || 0) + v;
...
self.onmessage = (e) => {
    ...
    if (e.data.type === 'PROCESS') {
        const base = calculate([], [], e.data.filters, e.data.metricKey, false); // Organic Baseline
        const impact = calculate(e.data.exclOff, e.data.exclTyp, e.data.filters, e.data.metricKey, true); // Simulated Impact
